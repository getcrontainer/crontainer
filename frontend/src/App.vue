        ...
        <section v-if="isList('schedules')" class="resource-section">
          ...
          <div class="data-panel not-format relative overflow-x-auto">
            ...
            <table class="resource-table w-full text-sm text-left">
              <thead class="text-sm font-bold uppercase text-gray-400"><tr><th class="px-6 py-3">Name</th><th class="px-6 py-3">Node</th><th class="py-3 text-center">Active</th><th class="px-6 py-3 w-36">Success rate</th><th class="px-6 py-3 w-36">Cron Rule</th><th class="px-6 py-3">Source</th><th class="px-6 py-3">CPU</th><th class="px-6 py-3">Memory</th><th class="px-6 py-3">Owner</th><th class="px-6 py-3 w-36">Action</th></tr></thead>
              <tbody><tr v-for="schedule in visibleCollections.schedules" :key="schedule.id" class="default-table-row"><td class="px-6 py-4 text-gray-900 rounded-s-xl"><div class="table-primary">{{ schedule.name }}</div><div class="table-secondary">{{ schedule.id }}</div></td><td class="px-6 py-4"><span class="provider-chip"><Monitor :size="14" aria-hidden="true" />{{ schedule.node_name || 'Unassigned' }}</span></td><td class="text-center"><span class="state-pill" :class="schedule.active ? 'state-active' : 'state-paused'"><span></span>{{ schedule.active ? 'Active' : 'Paused' }}</span></td><td class="px-6 py-4"><div class="success-rate" :class="successRateTone(schedule.success_rate)" :aria-label="`${formattedSuccessRate(schedule.success_rate)} success rate across the latest completed executions`"><div class="success-rate-value"><span class="success-rate-dot"></span><strong>{{ formattedSuccessRate(schedule.success_rate) }}</strong></div><div class="success-rate-track" aria-hidden="true"><span :style="{ width: `${normalizedSuccessRate(schedule.success_rate)}%` }"></span></div></div></td><td class="px-6 py-4"><code class="cron-code" :title="schedule.cron_description">{{ schedule.cron_rule }}</code></td><td class="px-6 py-4"><div class="flex items-center">
                <span
